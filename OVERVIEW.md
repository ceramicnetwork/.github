...
One of the primary decisions to make when designing a peer-to-peer data management system is how to properly handle *version control and state management* of documents. Such a system is important as it serves as the primary mechanism for handling conflicting updates without the use of a trusted server. We evaluated many different existing approaches to this problem, each possessing their own strengths, weaknesses, and use cases. For example, many blockchains employ a consensus algotithm which gurantees strict ordering of all transactions on the network over a global state, which provides strong security and finality guarantees but suffers from scalability issues. On the other hand, many peer-to-peer databases such as OrbitDB and content collaboration platforms such as Notion utilize a conflict-free replicated data type (CRDT) algorithm which provides good scalability by using eventually-consistent state management for each document but occassionally suffers from inconsistencies and provides no global state of all documents, since each document state is handled individually. As you can imagine, the difference is what makes blockchains great at managing financial assets and CRDT systems great at fostering realtime, multi-party collaboration.

When considering the design of the state management system for Ceramic, we wanted the best of both approaches: *security and scalability*. Ceramic documents should be able to securely manage data related to sensitive cryptographic operations such as document ownership changes and key rotations. These operations cannot tolerate eventual consistency and therefore require the strict ordering and finality provided by blockchains. However, the Ceramic network will collectively need to manage an extreme number of documents simultaneously and would quickly outgrow the scaling limitations experienced by blockchains with one ever-growing global state. For these reasons, for Ceramic we chose to implement a document-centric state management protocol which provides strict ordering over updates within each document without requiring the network to maintain any global state. We achieve this with DocState. DocState consists of timestamping (anchoring) a document's updates into a blockchain, causing each document to form it's own "doc-chain." By foregoing global state in favor of doc state, Ceramic can horizontally scale to handle the massive amount of information in the world without creating one ever-growing bloated ledger, while also allowing nodes to be run in resource-contstrained environments such as browsers and mobile/IoT devices.
