...
## Summary
[**Ceramic**](http://ceramic.network) is a peer-to-peer platform for building decentralized applications using smart documents. The Ceramic protocol allows anyone to permissionlessly store and manage dynamic content on the internet in a secure, collaborative, and trust-minimized manner. For a more technical overview, see [Ceramic Technical Specification](https://github.com/ceramicnetwork/specs).
...
One of the primary decisions to make when designing a peer-to-peer data management system is how to properly handle *version control and state management* of documents. Such a system is important as it serves as the primary mechanism for handling conflicting updates without the use of a trusted server. We evaluated many different existing approaches to this problem, each possessing their own strengths, weaknesses, and use cases. For example, many blockchains such as [Ethereum]() employ a consensus algotithm which gurantees strict ordering of all transactions on the network over a global state, which provides strong security and finality guarantees but suffers from scalability issues. On the other hand, many peer-to-peer databases such as [OrbitDB]() and content collaboration platforms such as [Notion]() utilize a conflict-free replicated data type (CRDT) algorithm which provides good scalability by using eventually-consistent state management for each document but occassionally suffers from inconsistencies and provides no global state of all documents, since each document state is handled individually. As you can imagine, the difference is what makes blockchains great at managing financial assets and CRDT systems great at fostering realtime, multi-party collaboration.

When considering the design of the state management system for Ceramic, we wanted the best of both approaches: *security and scalability*. Ceramic documents should be able to securely manage data related to sensitive cryptographic operations such as decentralized identities, document ownership changes, and key rotations. These operations cannot tolerate eventual consistency and therefore require the strict ordering and finality provided by blockchain consensus. However, the Ceramic network will collectively need to manage an extreme number of documents simultaneously and would quickly outgrow the scaling limitations experienced by blockchains with one ever-growing global state. For these reasons, on Ceramic we chose to implement a document-centric state management protocol which provides strict ordering over updates within each document without requiring the network to maintain any global state. We achieve this with *DocState*. DocState consists of timestamping (anchoring) a document's updates into a blockchain, causing each document to form it's own immutable "doc-chain." By foregoing global state in favor of DocState, Ceramic can horizontally scale to handle an extreme amount of information without creating one ever-growing bloated ledger, while also allowing nodes to be run in resource-contstrained environments such as browsers and mobile/IoT devices.


#### 1.3.2 Programmable State Transition Logic
In any peer-to-peer system that seeks to manage dynamic, mutable information another important consideration is how to handle document *state transitions* in a decentralzed but flexible way. State transition logic defines what constitutes a valid update to a document. For example, if an update is made to a given document that adheres to its defined state transition logic, then the update would be considered valid by the protocol and a new version would be created using the DocState system described above. Many peer-to-peer data management systems have a simply defined state transition ruleset that is generally applied to every document: if you are the owner of the document, you are allowed to make updates to it. However, there are lessons to be learned from smart contract blockchains such as Ethereum, which allow developers to program state transition logic for assets in a flexible way. For example, on Ethereum you could write a simple contract that says *do X if Y occurs*. State transition programmability is a powerful primitive that we have not yet seen applied to decentralized data, likely because most data systems are CRDT based and don't utilize strict ordering.

When designing the document state transition system for Ceramic, we wanted to ensure that documents could be programmed in the same way as smart contracts. This is the reason Ceramic documents are called *smart documents*. Ceramic allows developers to write programs that govern the state transition rules of a document with explicit logic and rules, which could fall anywhere on the spectrum from simple (update the document if signed by the owner(s)) to moderate (update the document if signed by the owner(s) and the update adheres to the document's defined content format and schema) to complex (update the document to Y if X external event happens such as the change in a related document). Ceramic achieves this with *doctypes*. Doctypes are software packages that contain state transition logic which can be applied to a document. On Ceramic, every document must specify a doctype and these rules will be enforced by the protocol. A number of community-developed doctypes can be found in Ceramic [CIPs](), and three doctypes are included by default in every node: [Tile (CIP-N)](), [3ID (CIP-N)](), and [Account Link (CIP-N)]().


#### 1.3.3 Data Discovery, Collaboration, and Composability
Most information on the internet today is trapped on proprietary cloud server-silos where it is safeguarded by a single gatekeeper and only provides utility for the application or service provider hosting the data. At the time when cloud was invented, it was a dramatic improvement over on-prem or local data storage solutions because it provided availability and removed the need for users to self-host. However, in order to access or update this information, a user or third-party would need to somehow discover the server, authenticate using an appliciation-specific login, and gather the schema (which is often proprietary) so the data could be interpreted. This application-centric (or server-centric) framework for managing data results in needless risk on data custodians, duplication of information and infrastructure, and limitation of access, while simultaneously restricting the cooperative ability for data to be easily discovered and shared across platforms. For many reasons beyond the scope of this paper, many developers are instead opting to build on new architectures without these limitations and costs.
